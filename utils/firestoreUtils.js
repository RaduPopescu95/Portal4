...
export const userLocation = `UsersUber/${
  ...
  try {
    ...
    if (actionText) {
      ...
      await handleUploadFirestoreSubcollection(
        actionData,
        `UsersUber/${currentUser?.uid}/ActiuniUber`,
        currentUser?.uid
      );
    }
  ...
  try {
    ...
    if (getNewData) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const ref = doc(
          db,
          `UsersUber/${data[i].collectionId}/Oferte/${data[i].documentId}`
        );
...
export const handleQueryFirestoreArray = async (
  collectionName,
  queryField,
  valueOne = null,
  queryFieldTwo = null,
  ...
  // Adăugăm referința la colecție
  conditions.push(collection(db, collectionName));

  // Câmpuri care sunt acum array-uri
  const arrayFields = [
    "titulaturaQ",
    "titulatura",
    "specialitate",
  ...
  if (valueOne) {
    // Dacă câmpul este un array, folosim "array-contains"
    const operator = arrayFields.includes(queryField) ? "array-contains" : "==";
    conditions.push(where(queryField, operator, valueOne));
  }

  if (valueTwo) {
    const operator = arrayFields.includes(queryFieldTwo) ? "array-contains" : "==";
    conditions.push(where(queryFieldTwo, operator, valueTwo));
  }
...
export const handlePaginateFirestore = (location) => {
  const auth = authentication;
  const citiesRef = collection(db, "UsersUber", auth.currentUser.uid, location);
